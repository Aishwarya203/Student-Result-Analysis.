...
if (strlen($_SESSION['alogin']) == "") {
    ...
    if (isset($_POST['submit'])) {
        $rowid = $_POST['id'];
        $internal_list = $_POST['internal'];
        $external_list = $_POST['exte'];
        $marks = $_POST['marks'];
        foreach ($_POST['id'] as $count => $id) {
            $internal = $internal_list[$count];
            $external = $external_list[$count];
            $mrks = $marks[$count];
            $iid = $rowid[$count];
            for ($i = 0; $i <= $count; $i++) {

                $sql = "update tblresult  set internal=:internal, exte=:external, marks=:mrks where id=:iid ";
                $query = $dbh->prepare($sql);
                $query->bindParam(':internal', $internal, PDO::PARAM_STR);
                $query->bindParam(':external', $external, PDO::PARAM_STR);
                                                ...
                                                ?>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="col-sm-10">
                                                                ...
                                                                <input type="text" name="internal[]" class="form-control" id="internal" value="<?php echo htmlentities($result->internal) ?>" maxlength="5" required="required" autocomplete="off">
                                                                <label for="internal">External</label>
                                                                <input type="text" name="exte[]" class="form-control" id="exte" value="<?php echo htmlentities($result->exte) ?>" maxlength="5" required="required" autocomplete="off">
                                                                <label for="internal">Total Marks</label>
